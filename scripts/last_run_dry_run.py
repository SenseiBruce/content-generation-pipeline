#!/usr/bin/env python3
"""Print dry_run for the most recent pipeline run."""
...
def last_run_dry_run(payload: dict[str, Any]) -> dict[str, Any]:
    runs = payload.get("runs")
    if not isinstance(runs, list) or not runs:
        return {"dry_run": None, "timestamp": None}
    last = runs[-1]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Print last pipeline run dry_run flag")
    parser.add_argument(
        "--state-file",
    ...
    if not isinstance(payload, dict):
        ...
        return 1
    print(json.dumps(last_run_dry_run(payload)))
    return 0
