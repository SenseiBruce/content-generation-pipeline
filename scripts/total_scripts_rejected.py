#!/usr/bin/env python3
"""Sum scripts_rejected across recorded pipeline runs."""
...
def total_scripts_rejected(state: dict[str, Any]) -> int:
    ...
    for run in runs:
        if not isinstance(run, dict):
            continue
        try:
            total += int(run.get("scripts_rejected") or 0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    json.dump({"scripts_rejected": total_scripts_rejected(load_state(args.state_file))}, sys.stdout)
    sys.stdout.write("\n")
    return 0
