...
def test_lists_keywords():
    assert list_losing_keywords({"losing_keywords": [" ipo ", "", 1, "nifty"]}) == [
        "ipo",
        "nifty",
    ]
...
def test_cli(tmp_path, capsys):
    ...
    path.write_text(json.dumps({"losing_keywords": ["gold"]}), encoding="utf-8")
    assert main(["--feedback-file", str(path)]) == 0
    assert json.loads(capsys.readouterr().out)["losing_keywords"] == ["gold"]
